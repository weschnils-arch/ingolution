...
export default function Contact() {
  ...
  useEffect(() => {
    ...
    const parallaxImg = section.querySelector('.contact-parallax-img') as HTMLElement | null
    const parallaxBand = section.querySelector('.contact-parallax-band') as HTMLElement | null
    if (parallaxImg && parallaxBand) {
      gsap.fromTo(
        parallaxImg,
        { yPercent: -15 },
        {
  ...
  return (
    <section id="contact" ref={sectionRef} className="bg-[var(--color-surface-2)]">
      {/* Parallax band — full bleed, slim */}
      <div className="contact-parallax-band contact-reveal relative w-full overflow-hidden h-[140px] md:h-[200px]">
        <img
          src="/contact-parallax.webp"
          alt=""
          aria-hidden="true"
          loading="lazy"
          decoding="async"
          className="contact-parallax-img absolute inset-0 w-full h-[140%] object-cover will-change-transform"
        />
        <div className="absolute inset-0 bg-gradient-to-b from-[var(--color-surface-2)]/40 via-transparent to-[var(--color-surface-2)]/60 pointer-events-none" />
      </div>
